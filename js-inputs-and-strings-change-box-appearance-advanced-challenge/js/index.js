...
const updateBoxProperty = (inputElement, property) => {
  inputElement.addEventListener("input", () => {
    const value = inputElement.value;
    if (property === "backgroundColor") {
      box.style[property] = `hsl(${value}, 70%, 60%)`;
    } else if (property === "transform") {
      box.style[property] = `rotate(${value}deg)`;
    } else {
...
updateBoxProperty(inputColor, "backgroundColor");
updateBoxProperty(inputRadius, "borderRadius");
updateBoxProperty(inputRotation, "transform");
